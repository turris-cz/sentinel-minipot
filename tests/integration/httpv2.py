...
def gen_mesg_report(ip, meth=b"", url=b"", auth=b"", user_ag=b""):
    ...
    if auth:
        data[AUTH] = auth
    if user_ag:
        data[USER_AG] = user_ag

    return proxy.gen_proxy_report2(TYPE, MSG_EV, ip, data)
...
def empty_mesg_test1(server_sock):
    ...
    msg = b"\n"
    send_to_sock(server_sock, msg)

    response = recv_from_sock(server_sock)
    str_cmp(response[0:len(BAD_REQ_PART1)], BAD_REQ_PART1)
    return reports
...
def req_line_test1(server_sock):
    ...
    str_cmp(response[0:len(UNAUTH_REQ_PART1)], UNAUTH_REQ_PART1)
...
def req_line_test7(server_sock):
    ...
    response = recv_from_sock(server_sock)
    str_cmp(response[0:len(URI_TOO_LONG_PART1)], URI_TOO_LONG_PART1)
