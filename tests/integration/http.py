...
def gen_meth(len):
    """ Returns randomly generated byte string from byte values <33, 126>.
...
def gen_url(len):
    """ Returns UTF-8 byte string generated randomly without byte values <0, 32> and 127.

    Parameters:
        len: int """
    ret = gen_rand_utf8_string(len)
    for i in range(33):
        ret = ret.replace(bytes([i]), bytes([randint(33, 126)]))
    ret = ret.replace(b"\x7f", bytes([randint(33, 126)]))
    return ret


def gen_head_name(len):
    """ Returns randomly generated byte string from byte values in <33, 126> without
    values 34, 40, 41, 44, 47, 58, 59, 60, 61, 62, 63, 64, 91, 92, 93, 123, 125.
...
def gen_head_val(len):
    """ Returns randomly generated byte string from values 9 and <32, 255> without
    value 127.

    Parameters:
        len: int """
    bytevals = list(range(32, 256))
    bytevals.append(9)  # add TAB
    bytevals.remove(127)  # remove DEL
    return bytes([choice(bytevals) for _ in range(len)])


def gen_user_ag(len):
    """ Returns UTF-8 byte string generated randomly without byte
    values <0, 32> and 127.
...
def gen_chunk_ext(len):
    """ Returns randomly generated byte string from byte values <32, 126>.
...
def gen_username(len):
    """ Returns UTF-8 byte string generated randomly without byte values 0 and 58.

    Parameters:
        len: int """
    ret = gen_rand_utf8_string(len)
    ret = ret.replace(b"\x00", bytes([randint(1, 127)]))
    ret = ret.replace(b":", bytes([randint(1, 57)]))
    return ret
...
def user_agent_head_test1(server_sock):
    ...
    method = gen_meth(500)
    url = gen_url(500)
    user_ag = gen_user_ag(randint(1, 2000))
...
def auth_header_test7(server_sock):
    ...
    method = gen_meth(500)
    url = gen_url(500)
    username = gen_username(500)
    password = gen_password(500)
...
def auth_header_test8(server_sock):
    ...
    method = gen_meth(500)
    url = gen_url(500)
    username1 = gen_username(randint(0, 500))
    password1 = gen_password(randint(0, 500))
    username2 = gen_username(randint(0, 500))
    password2 = gen_password(randint(0, 500))
    msg = b"".join([gen_req_line(method, url),
...
def auth_header_test9(server_sock):
    ...
    username = gen_username(500)
    password = gen_password(500)
    user_ag = gen_user_ag(randint(0, 2000))
    msg = b"".join([gen_req_line(method, url),
...
def auth_header_test10(server_sock):
    """ Sends to Minipots:
        valid request line
        valid authorization header
        username contains invalid utf-8 string
    Required response from Minipots':
        MINIPOT_UNAUTH_REQ_PART1
    Required Minipots generated Sentinel message:
        connect
    ...
    method = gen_meth(500)
    url = gen_url(500)
    username = gen_username(500) + b"\xf6"
    password = gen_password(500)
    msg = b"".join([gen_req_line(method, url),
                    gen_auth_header(user=username, passw=password), CRLF])
    server_sock.sendall(msg)
...
def auth_header_test11(server_sock):
    """ Sends to Minipots:
        valid request line
        valid authorization header
        password contains invalid utf-8 string
    Required response from Minipots':
        MINIPOT_UNAUTH_REQ_PART1
    Required Minipots generated Sentinel message:
        connect
    ...
    url = gen_url(500)
    username = gen_username(500)
    password = gen_password(500) + b"\xf7"
    msg = b"".join([gen_req_line(method, url),
                    gen_auth_header(user=username, passw=password), CRLF])
    server_sock.sendall(msg)
    response = recv_from_sock(server_sock)
...
def auth_header_test12(server_sock):
    """ Sends to Minipots:
        valid request line
        valid authorization header
        username contains invalid utf-8 string
    ...
    method = gen_meth(500)
    url = gen_url(500)
    username = gen_username(500) + b"\xf8"
    password = gen_password(500) + b"\xf7"
    msg = b"".join([gen_req_line(method, url),
                    gen_auth_header(user=username, passw=password), CRLF])
    server_sock.sendall(msg)
...
def brute_force_test1(server_sock):
    ...
    for i in range(MINIPOT_MESSAGES_LIMIT):
        method = gen_meth(randint(0, 600))
        url = gen_url(randint(1, 1000))
        username = gen_username(randint(0, 500))
        password = gen_password(randint(0, 500))
        user_ag = gen_user_ag(randint(0, 2000))
        msg = b"".join([gen_req_line(method, url),
                        gen_auth_header(user=username, passw=password),
                        gen_user_ag_header(user_ag), CRLF])
